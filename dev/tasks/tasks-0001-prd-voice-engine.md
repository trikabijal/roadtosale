...
- [ ] 1.0 **Repo + commit hygiene baseline**
  - [ ] 1.1 Commit the repo split (move auditpro/, design/ → demo/) on `chore/repo-split-demo-product`.
  - [ ] 1.2 Commit PRD + module docs on the same branch.
  - [ ] 1.3 Create new branch `feat/voice-engine-catalog-v1` for implementation.
  - [ ] 1.4 Create `dev/tasks/decisions-log.md` capturing all v1 decisions and the deferred-open-question status.

- [ ] 2.0 **Shared contracts (sequential prereq for everything downstream)**
  - [ ] 2.1 Define Python dataclasses for `TranscriptEvent`, `CueAtom`, `CueDetection` in `voice-engine/lab/src/voice_lab/types.py`.
  - [ ] 2.2 Define TS interfaces for the same in `voice-engine/src/types/index.ts` (`SessionContext` is TS-only).
  - [ ] 2.3 Define Python dataclasses for `Make`, `Model`, `Trim`, `Feature`, `TrimFeature`, `ValidationResult` in `vehicle-feature-catalog/src/python/vehicle_feature_catalog/entities.py`.
  - [ ] 2.4 Define TS interfaces for the same in `vehicle-feature-catalog/src/ts/entities.ts`.
  - [ ] 2.5 Commit shared contracts.

- [ ] 3.0 **Vehicle Feature Catalog implementation**
  - [ ] 3.1 Python: YAML loader walking `data/{makes,models,trims,features,matrix}/`.
  - [ ] 3.2 Python: in-memory indexes (`by_id`, `features_by_trim`, `trims_by_feature`).
  - [ ] 3.3 Python: `VehicleFeatureCatalog` facade with all methods spec'd in `api.md`.
  - [ ] 3.4 Python: `Validator` with the 5 integrity checks.
  - [ ] 3.5 Python: error classes (`NotFoundError`, `LoadError`, `DuplicateIdError`).
  - [ ] 3.6 Python: `scripts/validate.py` CLI wrapper.
  - [ ] 3.7 Python: unit tests for loader, facade, validator.
  - [ ] 3.8 TS: mirror of 3.1–3.6 in TypeScript.
  - [ ] 3.9 TS: unit tests for loader, facade, validator.
  - [ ] 3.10 `pyproject.toml`, `package.json`, `build.sh`, `README.md`.
  - [ ] 3.11 Cross-language drift test: load a known fixture with both loaders, assert identical JSON output.
  - [ ] 3.12 Commit catalog implementation.

- [ ] 4.0 **Vehicle Feature Catalog — seed data**
  - [ ] 4.1 Honda Make file.
  - [ ] 4.2 Honda CR-V Hybrid AWD 2026 Model file.
  - [ ] 4.3 Honda CR-V Hybrid AWD 2026 trims (Sport, Sport-L, Sport Touring per Honda US brochure).
  - [ ] 4.4 Universal feature files for the features the CR-V Hybrid exposes (Wireless Apple CarPlay, Wireless Android Auto, Heated Front Seats, Panoramic Moonroof, Hands-Free Power Tailgate, etc.).
  - [ ] 4.5 Honda brand-specific feature files (Honda Sensing 360+, Real-Time AWD with Intelligent Control, Bose Premium Audio if Honda-branded, HondaLink, etc.).
  - [ ] 4.6 `matrix/honda.yaml` — trim × feature ticks per the brochure.
  - [ ] 4.7 Toy Toyota fixture: 1 Make, 1 Model, 1 Trim, 1 brand-specific feature, shared universal features.
  - [ ] 4.8 `matrix/toyota.yaml`.
  - [ ] 4.9 Run `voice-lab validate-catalog` (after catalog impl lands) — must pass.
  - [ ] 4.10 Commit seed data.

- [ ] 5.0 **Voice Engine core — strategy + matcher + facade (Python + TS)**
  - [ ] 5.1 Python: `TranscriptionStrategy` ABC in `strategies/base.py`.
  - [ ] 5.2 Python: strategy registry in `strategies/registry.py`.
  - [ ] 5.3 Python: `cue_matcher.py` — phrase + synonym matching against transcript stream.
  - [ ] 5.4 Python: `VoiceEngineLab` facade with `list_strategies`, `get_strategy`, `transcribe_file`, `match_cues`.
  - [ ] 5.5 Python: `MockTranscriptionStrategy` (replays a JSONL event file — used by tests).
  - [ ] 5.6 Python: stub `AppleSpeechTranscriberStrategy` and `ArgmaxStrategy` — registered but raise `NotImplementedError` with a pointer to OQ1 / OQ2.
  - [ ] 5.7 Python: unit tests for matcher, facade, mock strategy.
  - [ ] 5.8 TS: `TranscriptionStrategy` interface in `strategies/base.ts`.
  - [ ] 5.9 TS: strategy registry in `strategies/registry.ts`.
  - [ ] 5.10 TS: `cue-matcher.ts`.
  - [ ] 5.11 TS: `VoiceEngine` facade with `listStrategies`, `getStrategy`, `startSession`, `matchCues`.
  - [ ] 5.12 TS: `MockTranscriptionStrategy` (live event replay).
  - [ ] 5.13 TS: stub `AppleSpeechTranscriberStrategy` — registered but throws `NotImplementedError` with pointer to OQ1.
  - [ ] 5.14 TS: unit tests for matcher, facade, mock strategy.
  - [ ] 5.15 `voice-engine/package.json`, `voice-engine/build.sh`, `voice-engine/lab/build.sh`, `voice-engine/lab/pyproject.toml`.
  - [ ] 5.16 iOS native module placeholder (`voice-engine/ios/Sources/AppleSpeechTranscriberModule.swift`).
  - [ ] 5.17 Android stub directory.
  - [ ] 5.18 Cross-language drift test (Python `TranscriptEvent` round-trips through TS loader, fields identical).
  - [ ] 5.19 Commit voice engine core.

- [ ] 6.0 **Universal workflow cues + drafted scripts**
  - [ ] 6.1 Draft `voice-engine/lab/cue-packs/universal_workflow_cues.yaml` from the audio script in `demo/auditpro-rn-showcase/src/services/audio/audioScript.ts` + the test matrix doc.
  - [ ] 6.2 Draft 2 example scripts targeting Honda CR-V Hybrid AWD 2026 Sport Touring (one positive, one with a deliberately omitted cue → negative).
  - [ ] 6.3 Commit cue inventory + sample scripts.

- [ ] 7.0 **Lab orchestrator + scoring + reporting + CLI (Python only)**
  - [ ] 7.1 `orchestrator.py` — composes catalog + engine, runs the per-script loop, projects `Feature → CueAtom`, merges with universal atoms.
  - [ ] 7.2 `scoring/classify.py` — pass/partial/fail rules per `dev/docs/ROAD_TO_SALE_AUDIO_TEST_MATRIX.md`.
  - [ ] 7.3 `scoring/latency.py` — P50 / P95 / P99 computation per strategy.
  - [ ] 7.4 `synthesis/base.py`, `synthesis/elevenlabs.py` (stub — flagged for OQ3 voice IDs + API key), `synthesis/noise.py` (stub — flagged for OQ7 noise track).
  - [ ] 7.5 `reporting/markdown.py` — `summary.md` writer.
  - [ ] 7.6 `reporting/csv_writer.py` — per-strategy CSV + false-positives CSV.
  - [ ] 7.7 `cli.py` — `synth`, `run`, `validate-catalog` subcommands.
  - [ ] 7.8 Unit tests for orchestrator (with mock strategy), scoring, reporting.
  - [ ] 7.9 Smoke test: end-to-end `voice-lab run --strategies mock` produces a valid report against the 2 drafted scripts using the seed catalog.
  - [ ] 7.10 Commit lab orchestrator.

- [ ] 8.0 **Cross-cutting glue**
  - [ ] 8.1 Static import check script (`scripts/check_imports.py` at repo root or per-module) — fails if `voice-engine/` imports from `vehicle-feature-catalog/` or vice versa.
  - [ ] 8.2 Top-level README updated with the new layout.
  - [ ] 8.3 `voice-engine/README.md`, `vehicle-feature-catalog/README.md`, `voice-engine/lab/README.md` — one-line purpose + how to build / test.
  - [ ] 8.4 Final decisions-log update for anything decided during build.
  - [ ] 8.5 Commit glue + final docs.

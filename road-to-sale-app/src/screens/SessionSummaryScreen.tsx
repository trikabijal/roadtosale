...
function generateHtmlReport(session: Session): string {
  const firstName = session.customer.firstName;
  const lastName = session.customer.lastName ?? '';
  const customerName = `${firstName}${lastName ? ' ' + lastName : ''}`.trim();
  const vehicleLabel = `${session.vehicle.year} ${session.vehicle.makeName} ${session.vehicle.modelName} ${session.vehicle.trimName}`;
  ...
  const stepsHtml = session.checklist.steps
    .map((step: StepState) => {
      const done = step.questions.filter(
        (q: QuestionState) => q.status === 'complete' || q.status === 'overridden',
      ).length;
      const stepTitle = `${step.isComplete ? '&#10003;' : '&#9675;'} Step ${step.header.orderNo}: ${escapeHtml(step.header.name)} (${done}/${step.questions.length})`;
      const questionsHtml = step.questions
        .map((q: QuestionState) => {
          let cls = 'pending';
          let icon = '&#9675;';
          if (q.status === 'complete') { cls = 'complete'; icon = '&#10003;'; }
          else if (q.status === 'overridden') { cls = 'overridden'; icon = 'M'; }
          return `<div class="question ${cls}">${icon} ${escapeHtml(q.question.question)}</div>`;
        })
        .join('');
      return `<div class="step"><div class="step-header">${stepTitle}</div>${questionsHtml}</div>`;
    })
    .join('');
...
<head>
  ...
  <style>
    ...
    h1 { font-size: 20px; color: #0066CC; }
    h2 { font-size: 16px; margin-top: 20px; border-bottom: 1px solid #ddd; padding-bottom: 4px; }
    .hero { background: #f0f7ff; padding: 16px; border-radius: 8px; margin-bottom: 16px; }
    .step { margin-top: 12px; }
    .step-header { font-weight: bold; }
    .question { margin-left: 16px; margin-top: 4px; font-size: 13px; }
    .complete { color: #16a34a; }
    .overridden { color: #7c3aed; }
...
<body>
  <div class="hero">
    <h1>Road to Sale — Session Report</h1>
    <p><strong>Customer:</strong> ${escapeHtml(customerName)}</p>
    <p><strong>Vehicle:</strong> ${escapeHtml(vehicleLabel)}</p>
    <p><strong>Date:</strong> ${escapeHtml(dateLabel)}</p>
    <p><strong>Completed:</strong> ${session.checklist.completedCount} / ${session.checklist.totalCount} questions</p>
  </div>
  <h2>NADA Checklist</h2>
  ${stepsHtml}
  <div class="footer">Generated by Road to Sale · ${escapeHtml(timestamp)}</div>
</body>
</html>`;
}
...
export default function SessionSummaryScreen({
  ...
  async function handleExportPdf() {
    ...
    setSharing(true);
    try {
      // Primary path: PDF via expo-print
      ...
      // Fallback: plain-text share via expo-file-system if PDF export fails
      try {
        const text = generateReport(session);
        const available = await Sharing.isAvailableAsync();
        if (available) {
          // expo-file-system v56: use new File + Paths API
        ...
        Alert.alert(
          'Export Failed',
          fallbackErr instanceof Error ? fallbackErr.message : 'Could not generate report.',
        );
      }
      // Surface the original PDF error to aid debugging
      if (pdfErr instanceof Error) {
        console.warn('[SessionSummaryScreen] PDF export failed, used text fallback:', pdfErr.message);
      }
    } finally {
      setSharing(false);
    }
